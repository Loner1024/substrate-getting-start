...
pub mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn creat_claim(origin: OriginFor<T>, claim: Vec<u8>) -> DispatchResultWithPostInfo {
            ...
            let bounded_claim = BoundedVec::try_from(claim).map_err(|_| Error::<T>::ClaimTooLong)?;
            ensure!(!Proofs::<T>::contains_key(&bounded_claim), Error::<T>::ProofAlreadyExist);
            Proofs::<T>::insert(&bounded_claim, (&sender, frame_system::Pallet::<T>::block_number()));
            Self::deposit_event(Event::ClaimCreated(sender, bounded_claim.into_inner()));
            Ok(().into())
        }
        ...
        pub fn revoke_claim(origin: OriginFor<T>, claim: Vec<u8>) -> DispatchResultWithPostInfo {
            ...
            Self::deposit_event(Event::ClaimRevoked(sender, bounded_claim.into_inner()));
            Ok(().into())
        }
        ...
        pub fn transfer_claim(origin: OriginFor<T>, claim: Vec<u8>, dest: T::AccountId) -> DispatchResultWithPostInfo {
            ...
            Self::deposit_event(Event::ClaimTransferred(sender, dest, bounded_claim.into_inner()));
            Ok(().into())
        }
